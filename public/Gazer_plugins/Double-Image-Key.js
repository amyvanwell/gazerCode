...
function finishdoubleImageKey(info) {
    ...
    let keyResp = info;
    if (keyResp != "none") {
        keyResp = jsPsych.pluginAPI.convertKeyCodeToKeyCharacter(info.key);
    }
...
jsPsych.plugins['Double-Image-Key'] = function () {
    ...
    plugin.info = {
        ...
        parameters: {
            duration: {
                ...
                description: 'Max to display the image in ms, defaults to 60 seconds.',
                default: 5000
            },
            stimulusOne: {
            ...
            endKeys: {
                type: jsPsych.plugins.parameterType.STRING,
                description: 'The keys used to end the trial, can be a single string or a list. Defaults to "f" and "j" keys.',
                default: ["f", "j"]
            },
            paddingVal: {
                type: jsPsych.plugins.parameterType.INTEGER,
    ...
    plugin.trial = function (display_element, trial) {
        ...
        display_element.innerHTML = display_element.innerHTML = "<div id='main'><div id='cross' style='position:absolute;font-size:60px;'>+</div>" + "<img id='stimulusOne' style='visibility:hidden; padding-right:" + paddingVal + "px;' src='" + stimulusOne + "'>" + "<img id='stimulusTwo' style='visibility:hidden; padding-left:" + paddingVal + "px;' src='" + stimulusTwo + "'>" + "</div>";
        ...
        manualKeyResp = jsPsych.pluginAPI.getKeyboardResponse({
